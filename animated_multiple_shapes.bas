...
320   VDU 25, 0, x4; y4;
330   VDU 25, 101, x4+50; y4+30;
340   REM Update positions
350   x1 = x1 + dx1 : y1 = y1 + dy1
360   x2 = x2 + dx2 : y2 = y2 + dy2
370   x3 = x3 + dx3 : y3 = y3 + dy3
380   x4 = x4 + dx4 : y4 = y4 + dy4
390   REM Bounce off edges
400   IF x1 <= 0 OR x1+40 >= 1240 THEN dx1 = -dx1
410   IF y1 <= 0 OR y1+40 >= 1024 THEN dy1 = -dy1
420   IF x2 <= 0 OR x2+40 >= 1240 THEN dx2 = -dx2
430   IF y2 <= 0 OR y2+40 >= 1024 THEN dy2 = -dy2
440   IF x3 <= 0 OR x3+40 >= 1240 THEN dx3 = -dx3
450   IF y3 <= 0 OR y3+40 >= 1024 THEN dy3 = -dy3
460   IF x4 <= 0 OR x4+50 >= 1240 THEN dx4 = -dx4
470   IF y4 <= 0 OR y4+30 >= 1024 THEN dy4 = -dy4
